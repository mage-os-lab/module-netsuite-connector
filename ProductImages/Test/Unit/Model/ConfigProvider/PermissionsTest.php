...
namespace MageOS\NetSuiteConnector\ProductImages\Test\Unit\Model\ConfigProvider;
...
/**
 * Covers the product image import feature flag.
 */
class PermissionsTest extends TestCase
{
    private const GET_PRODUCT_IMAGES_PATH = 'mageos_netsuite/enable_disable_features/get_product_images';
    ...
    public function testItReturnsTheConfiguredFlagForTheKnownFeatureCode(bool $flag): void
    {
        ...
        $this->assertSame($flag, $permissions->isFeatureEnabled(Permissions::GET_PRODUCT_IMAGES));
    }
    ...
    public static function unknownFeatureCodeProvider(): array
    {
        return [
            'empty code' => [''],
            'code owned by another module' => ['get_products'],
            'singular near miss' => ['get_product_image'],
            'full path instead of a code' => [self::GET_PRODUCT_IMAGES_PATH],
        ];
    }
    ...
    public function testItQueriesTheExactConfigurationPath(): void
    {
        ...
        $scopeConfig->expects($this->once())
            ->method('isSetFlag')
            ->with(self::GET_PRODUCT_IMAGES_PATH)
            ->willReturn(true);
        $permissions = new Permissions($scopeConfig);

        $this->assertTrue($permissions->isFeatureEnabled(Permissions::GET_PRODUCT_IMAGES));
    }
    ...
    public function testItDoesNotTouchScopeConfigForAnUnknownFeatureCode(): void
    {
        ...
        $this->assertFalse($permissions->isFeatureEnabled('not_a_product_image_feature'));
    }
}
