...
namespace MageOS\NetSuiteConnector\Shipment\Test\Unit\Model\ConfigProvider;

use Magento\Framework\App\Config\ScopeConfigInterface;
use MageOS\NetSuiteConnector\Shipment\Model\ConfigProvider\Permissions;
use PHPUnit\Framework\Attributes\DataProvider;
use PHPUnit\Framework\TestCase;

/**
 * Covers the shipment import feature flag.
 */
class PermissionsTest extends TestCase
{
    private const GET_SHIPMENTS_PATH = 'mageos_netsuite/enable_disable_features/get_shipments';
    ...
    public function testItReturnsTheConfiguredFlagForTheKnownFeatureCode(bool $flag): void
    {
        ...
        $this->assertSame($flag, $permissions->isFeatureEnabled(Permissions::GET_SHIPMENTS));
    }
    ...
    public static function unknownFeatureCodeProvider(): array
    {
        return [
            'empty code' => [''],
            'code owned by another module' => ['get_order_changes'],
            'singular near miss' => ['get_shipment'],
            'full path instead of a code' => [self::GET_SHIPMENTS_PATH],
        ];
    }
    ...
    public function testItQueriesTheExactConfigurationPath(): void
    {
        ...
        $scopeConfig->expects($this->once())
            ->method('isSetFlag')
            ->with(self::GET_SHIPMENTS_PATH)
            ->willReturn(true);
        $permissions = new Permissions($scopeConfig);

        $this->assertTrue($permissions->isFeatureEnabled(Permissions::GET_SHIPMENTS));
    }
    ...
    public function testItDoesNotTouchScopeConfigForAnUnknownFeatureCode(): void
    {
        ...
        $this->assertFalse($permissions->isFeatureEnabled('not_a_shipment_feature'));
    }
}
